...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            {{-- Export Excel --}}
            <a href="{{ route('export-paket') }}" style="color:white">
            ...
            {{-- Export PDF --}}
            <a href="{{ route('exportPDF-paket') }}" style="color:white" target="_blank">
                ...
                <tbody>
                    @foreach ($paket as $pk)
                        <tr>
                            ...
                            <td>
                                <!-- Update Data -->
                                <button class="btn edit-paket btn-success" type="button" data-toggle="modal"
                                    data-target="#IsiBarang" data-mode="edit" data-id="{{ $pk->id }}"
                                    data-id_outlet="{{ $pk->id_outlet }}" data-jenis="{{ $pk->jenis }}"
                                    data-nama_paket="{{ $pk->nama_paket }}"
                                    data-harga="{{ $pk->harga }}"><a>UPDATE</a></button>

                                <!-- Delete Data -->
                                <form action="/{{ request()->segment(1) }}/paket/{{ $pk->id }}" method="POST"
                ...
                $('#IsiBarang').on('show.bs.modal', function(event) {
                    ...
                    if (mode === "edit") {
                        ...
                        modal.find('.modal-body form').attr('action',
                            '{{ url(request()->segment(1)) }}/paket/' + id)
