...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            {{-- Export PDF --}}
            <a href="{{ route('exportPDF-baranginventaris') }}" style="color:white" target="_blank">
                        ...
                        -->
                            ...
                            <td>
                                ...
                                <button class="btn edit-member btn-success" type="button" data-toggle="modal"
                                    data-target="#IsiBarang" data-mode="edit" data-id="{{ $bi->id }}"
                                    data-nama_barang="{{ $bi->nama_barang }}" data-merk_barang="{{ $bi->merk_barang }}"
                                    data-qty="{{ $bi->qty }}" data-kondisi="{{ $bi->kondisi }}"
                                    data-tanggal_pengadaan="{{ $bi->tanggal_pengadaan }}"><a>UPDATE</a></button>

                                <!-- Delete Data -->
                                <form action="{{ route('barang_investaris.destroy', $bi->id) }}" method="POST"
                ...
                $('#IsiBarang').on('show.bs.modal', function(event) {
                    ...
                    if (mode === "edit") {
                        ...
                        modal.find('.modal-body form').attr('action',
                            '{{ url(request()->segment(1)) }}/barang_investaris/' + id)
